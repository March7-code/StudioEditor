...
```text
Assets/StudioEditor/Rendering/
  Core/
    StudioEditor.Rendering.asmdef
    CharacterRenderMaterialContext.cs
    CharacterRenderSchemeRegistry.cs
    MaterialRenderUtility.cs
  RenderSchemes/
    DefaultAnime/
      StudioEditor.RenderScheme.DefaultAnime.asmdef
      DefaultAnimeCharacterRenderScheme.cs
      Resources/Shaders/StudioEditorAnimeCharacter.shader
```
...
Koikatsu shader ramp textures are not interchangeable with the StudioEditor toon
...
segments. Sampling it as StudioEditor's `(halfLambert, rampRow)` lookup produces a
large false shadow that resembles broken normals.

The current compatibility rule is therefore:

- Do not infer StudioEditor ramp compatibility from source property names such as
...
StudioEditor ramp or select a dedicated shader path. Texture-property name matching
